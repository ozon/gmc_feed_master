...
export function RuleCard({
  ...
  const count = parseIdList(draft).size;
  ...
  return (
    <Accordion.Item value={rule.id} data-testid={`rule-card-${rule.id}`}>
      <Accordion.Control onClick={() => setOpen((o) => !o)}>
      ...
      {open && (
        <div className="mantine-Accordion-panel-focusable" data-testid={`rule-panel-${rule.id}`}>
          <Stack gap="xs" pt="xs">
              ...
              <Stack gap={4}>
                <Textarea
                  ...
                  onChange={(e) => {
                    ...
                    onSetIds(e.currentTarget.value);
                  }}
