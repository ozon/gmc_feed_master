...
import { chartColors, donutPalette } from '../../components/dashboard/dashboardColors';
...
export function FleetCharts({ summary }: { summary: DashboardSummary }) {
  ...
  const feeds = summary.clients.flatMap((client) =>
    client.feed_sources.map((feed) => ({
      name: `${client.name} / ${feed.name}`,
      value: feed.item_count,
    })),
  );
  ...
  const donutData = [
    ...
    ...(rest.length > 0
      ? [
          {
            name: t('charts.other'),
            value: rest.reduce((sum, f) => sum + f.value, 0),
  ...
  return (
    <Grid>
      <Grid.Col span={{ base: 12, md: 5 }}>
        ...
        >
          <DonutChart size={180} data={donutData} withTooltip mx="auto" />
        </ChartCard>
      </Grid.Col>
      <Grid.Col span={{ base: 12, md: 7 }}>
        <ChartCard
          title={t('charts.healthTitle')}
          isEmpty={trend.length === 0}
          emptyMessage={t('charts.healthEmpty')}
        >
          <AreaChart
            h={220}
            data={trend}
