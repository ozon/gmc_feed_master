...
async def _seed_rows(factory) -> None:
    ...
    old_usage = AiUsageLog(
        ...
        provider_config_id=None, model="m", cache_hit=False,
    ...
    async with factory() as session, session.begin():
        session.add_all([old_usage, fresh_usage])
        await session.flush()
...
async def test_purge_respects_configured_retention(session_factory):
    ...
    counts = await purge_expired_ai(session_factory, NOW)
    assert counts == AiPurgeCounts(usage_rows=0)
