...
| 5. AI Rules (opt-in) | `RuleAiStep` | When `configuration.ai_rules = {enabled, limit, budget}` is set, drain the `op=ai` rule actions `PluginStep` recorded on `RunState.rule_ai_pending` through `AiService` (pinned template via `run_task(template_id=…)` or custom prompt via `run_inline_task`), write the resolved target fields into the product, and persist to staging `processed_data` (dry-run stays in memory). `ok` spends one budget unit, `cache_hit` is free, `fallback`/raised errors count as failures and never abort other products. Disabled or no pending actions → no-op |
| 6. AI Enrichment (opt-in) | `EnrichmentStep` | When `configuration.ai_enrichment = {enabled, tasks, limit, budget}` is set, call `AiService.run_task` per product for each configured task (`title_optimization`, `description_optimization`, `category_classification`, `attribute_enrichment`) and write results to the `enrichment` plugin's feed-source `PluginData.suggestions` (pinned values preserved). Nothing changes product output until a human accepts fields in the Enrichment UI. `ok` spends one budget unit, `cache_hit` is free, `fallback` counts as a failure; one product's failure never aborts the batch. Disabled (default) → no-op |
...
Swappable LLM interface shared by all AI features. `AiService.run_task(task_type, variables)` resolves a task from the registry (`app/ai/tasks.py`: title_optimization, description_optimization, category_classification, policy_check, attribute_enrichment, image_quality) → renders the prompt (DB template or builtin) → LiteLLM-native cache lookup (per-task namespace + TTL, validated-only writes) → `litellm.Router` call wrapped by Instructor structured output → validated Pydantic model → usage log. Constructed in the app lifespan, attached to `app.state.ai_service`. Consumed by the pipeline's `QualityCheckStep` via the `AiPolicyCheck` cross-product rule (Z3) when a feed source enables `configuration.ai_qc`.

Key properties:
- **Transport**: a `litellm.Router` is built from the enabled `ai_provider_configs` rows, grouped by `tier` (`bulk` / `precision`) into model groups; `fallbacks=[{"bulk": ["precision"]}]` gives automatic failover. Retry count, allowed failures, cooldown, and timeout come from the `global_settings` AI row. Legacy `openai_compatible` rows map to `openai/<model>` with `api_base`.
- **Structured output**: `instructor.from_litellm(router.acompletion, async_client=True)`. Each task declares a Pydantic `response_model` (`app/ai/schemas.py`), so every result is a validated model instance and a schema violation triggers a bounded reask (`ai_instructor_max_retries`). Chat is the exception — free-form content plus `tool_calls`, no schema.
- **Never blocks the pipeline**: provider errors, router-build failures, and invalid output all return `AiResult(status="fallback", error_code=...)` — the caller decides its own non-AI fallback value.
- **Caching (LiteLLM native)**: backend `local` / `disk` / `redis` (`app/ai/cache_config.py`), namespaced per task type (`{namespace}:{task_type}`) with a long TTL for taxonomy tasks and a moderate TTL for content tasks. Only validated responses are written: the Router call is deliberately *not* given cache-control (no `caching=True`/`cache_responses`), and the manual `async_get_cache`/`async_add_cache` calls opt in with `cache={"use-cache": True, ...}` — without that flag LiteLLM's `CacheMode.default_off` makes both no-op. Reads are re-validated through the Pydantic model before use. Cache backend errors are fail-open (log + call directly). Redis is selected by `REDIS_URL` (or discrete `REDIS_HOST`/`REDIS_PORT`/`REDIS_PASSWORD`/`REDIS_SSL`), and `rediss://` enables TLS.
- **Admin settings (phase C)**: `GET/PUT /admin/ai/settings` reads/edits the `global_settings` AI row. `PUT` persists then rebuilds the LiteLLM cache and Router and swaps them onto the running `AiService`; a build failure returns 422 and the previous config stays active (the DB write rolls back). `GET /admin/ai/cache` (status/health/entries), `GET /admin/ai/cache/stats` (hit ratio + saved tokens/cost), `POST /admin/ai/cache/clear` (namespace-scoped), and `GET /admin/ai/usage/{summary,timeseries}` complete the surface.
- **Cost tracking**: one `ai_usage_logs` row per call (including cache hits, tokens 0), carrying `provider` / `tier` / `fallback_used`; aggregated by `GET /admin/ai/usage`.
- **Model catalog**: `app/ai/model_catalog.py` snapshot of LiteLLM's `model_prices_and_context_window.json`. Lazy-seeds from the installed `litellm.model_cost` when empty, refreshed from the upstream raw JSON by the daily `system-ai-model-catalog-refresh` job (`0 4 * * *`) or `POST /admin/ai/model-catalog/refresh`. A failed refresh keeps the last-good catalog and records the error; the catalog never blocks provider use. `GET /admin/ai/provider-presets` supplies the admin wizard's provider list.
...
- `enforce_scope_access` — router-level dependency on the clients/products/pipeline/quality/dry-run/export-history/field-mapping/plugins routers and on every plugin-contributed router: reads `client_id`/`feed_source_id` from path AND query params (plugin scope params are query params), validates **every** identifier present (a valid `client_id` never short-circuits the `feed_source_id` check), resolves feed source → client, returns 404 for unassigned resources (no existence leak). Admins and the non-DB fallback pass through.
