...
- **Caching (LiteLLM native)**: backend `local` / `disk` / `redis` (`app/ai/cache_config.py`), namespaced per task type (`{namespace}:{task_type}`) with a long TTL for taxonomy tasks and a moderate TTL for content tasks. Only validated responses are written; reads are re-validated through the Pydantic model before use. Cache backend errors are fail-open (log + call directly). Redis is selected purely by `REDIS_URL` in the environment.
