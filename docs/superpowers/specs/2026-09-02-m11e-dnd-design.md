...
**Uniqueness argument (why deterministic ids cannot collide):** ids collide only on a duplicate `(plugin_id, position)` pair. `position` is unique by construction: server lists are index-normalized (§ toServer), hydration derives from those positions, and appends use `instances.length` — the next free index — which can never equal an in-use index of the same plugin... except one edge: remove-then-append where the removal frees index `n` and `instances.length` reuses it for the SAME plugin id. Result: the new card takes the freed id. That is correct behavior, not a collision — the previous holder is gone (`removeInstance` filtered it out), so the id is free again. React key uniqueness holds because the map is over the CURRENT list. No duplicate key can exist in a single rendered list.
...
- NEW: "reorder and remove preserve ids" — `reorderInstances([a,b,c],0,2)` keeps each item's own `clientId` (just moved); `removeInstance([a,b,c],'b')` leaves `a` and `c` ids intact; `addInstance(removeInstance([p0,p1-as-x], x), p)` mints the freed-index id, not a random one.
