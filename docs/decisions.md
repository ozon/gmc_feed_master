...
- **CI baseline-gate design:** `ruff check` and `mypy` use count-file baselines (`backend/ruff-baseline.txt`, `backend/docs/mypy-baseline.md`); the gate scripts count errors and flip to exit-0 when the count equals the baseline, enforcing zero-new-errors without fixing pre-existing issues. (Path correction 2026-09-11: the ruff baseline lives at `backend/ruff-baseline.txt`, count 508 — earlier entries citing `docs/ruff-baseline.txt`/506 predate the file's final location and count.)
...
### 2026-09-12 — Prompt library UI (Feature 3 of AI integration)

**Topic:** Admin frontend for the prompt template library.
...
### 2026-09-16 — AI core replacement phase A (LiteLLM + Instructor)

**Topic:** Replace the bespoke AI transport, loose JSON validators, and DB result cache with LiteLLM Router + Instructor + LiteLLM native cache, behind an unchanged `AiService` public API, with full feature parity.

**Decision:**
- **LiteLLM Router is the sole transport.** `app/ai/router.py` builds a Router from enabled `ai_provider_configs` rows grouped by `tier` (`bulk`/`precision`), `fallbacks=[{"bulk": ["precision"]}]`, and router retry/cooldown/timeout from the `global_settings` AI row. Legacy `openai_compatible` rows map to `openai/<model>` + `api_base`. `tier` replaces `is_default` (dropped from the API/UI). Full rationale and spec deviations: `docs/decisions/0010-litellm-instructor-ai-transport.md`.
- **Instructor is the sole structured-output path.** Each task declares a Pydantic `response_model` (`app/ai/schemas.py`); schema violations trigger a bounded reask (`ai_instructor_max_retries`). `validate_task` is removed. Chat stays free-form (`tool_calls`, no schema). Property/GTIN/description constraints come from `registry/attributes.json`; category ids validate against the category plugin's taxonomy CSV; promotional/all-caps rejection is a title-field validator (no equivalent existed in `qc/rules.py`, so it was not duplicated there).
- **Idempotency of the cutover.** The DB cache key embedded `provider_config_id`/`model`, which tier routing invalidates — so the native cache shipped in the same phase as the transport rather than a follow-up. Instructor runs with `CacheMode.default_off`; only validated payloads are written, and reads are re-validated (a cache hit that fails validation is treated as a miss). Cache errors are fail-open.
- **Additive migration (m15).** Adds `ai_provider_configs.tier`, the `global_settings` AI columns (`ai_cache_type`, `ai_cache_namespace`, TTLs, router knobs, `ai_instructor_max_retries`), and `ai_usage_logs.provider/tier/fallback_used`. The physical drops (`is_default`, `ai_result_cache`, `ai_cache_retention_days`) and deletion of `openai_compat.py`/`cache.py`/`resilience.py`/`AIProvider` move to the cleanup phase, keeping every phase-A commit green while old code is still referenced. `alembic check` was already failing on unrelated `feed_sources`/`staging_products` index drift at HEAD; the generated migration was hand-trimmed to AI-only operations.
- **Config split.** Model ids/keys remain in `ai_provider_configs` (DB). `.env` holds only `REDIS_URL` and `AI_CACHE_DIR`; the rest of the AI knobs are DB-backed for a later admin-editable surface.
...
### 2026-09-16 — AI core cleanup (phase D)

**Topic:** Delete the retired AI transport/cache modules and drop the now-unused persistence, completing the LiteLLM + Instructor replacement.

**Decision:**
- **Deleted `app/ai/openai_compat.py`, `app/ai/cache.py`, `app/ai/resilience.py`** and their tests. `app/ai/provider.py` is trimmed to the `AiResponse` chat DTO (the `AIProvider` Protocol and `AiRequest` are gone). `app/ai/__init__.py` exports only live symbols.
- **Removed dead `estimate_cost`** from `app/ai/usage.py` — nothing had called it since the phase-A cutover (cost estimates now come from LiteLLM/provider usage).
- **Migration `m16` (`b1a2c3d4e5f6`) drops** `ai_result_cache`, `ai_provider_configs.is_default`, and `global_settings.ai_cache_retention_days`. Hand-written (not autogenerated) because HEAD already carries unrelated `feed_sources`/`staging_products` index drift that autogenerate would sweep in.
- **`purge_expired_ai` is usage-only** (`AiPurgeCounts(usage_rows)`), and the nightly job log line no longer references cache rows.
