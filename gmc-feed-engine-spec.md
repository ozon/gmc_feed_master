...
| Custom labels | Labelizer plugin with slot rules (config: global + client scope, merged union-by-id; data: client + feed_source per-rule value lists). No separate label matrix entity |
...
Unchanged (both hashes match) → only update `last_seen_at`, no re-run of the pipeline. Changed → re-run the pipeline for that product. Any plugin config, data, or version change therefore triggers reprocessing of the affected products on the next run — no manual trigger needed. Note: a **global**-scope config change (e.g. a shared Labelizer slot rule) changes the resolved config for every feed source of every client using that plugin, and therefore triggers full reprocessing across all of them on their next run — an accepted trade-off for MVP simplicity over a more granular, per-scope hash.
...
2. **PluginConfig** — structural configuration, rarely changed (e.g. Labelizer slot rules). Validated against `config_schema`.
...
**Scope resolution (three-tier, generic — applies to any plugin declaring more than one scope in its manifest):** `feed_source` wins where present, `client` is next, `global` is the fallback/merge base. The merge is per key: a key missing at `feed_source` level is taken from `client`; missing there too, from `global`. The generic merge replaces non-dict values per key — the more specific scope wins wholesale; a plugin may instead declare a per-key list-merge strategy in its manifest (`config_merge`, strategy `union_by_key`) so ordered lists union by entry key — ancestor entries keep their positions, more-specific tiers override entries with the same key, unseen entries are appended (used by Labelizer `slotRules`, ADR-0005). A plugin need not declare all three scopes, and config/data scopes may differ — e.g. Labelizer declares `["global", "client"]` for config (shared slot rules) but `["client", "feed_source"]` for data (per-market value lists); Category (planned) declares only `["global", "client"]`. `feed_source`-scoped configuration only applies to plugins that declare it.
...
1. **Labelizer** — populates `custom_label_0`–`custom_label_4` via **slot rules** (PluginConfig, scopes global + client, merged union-by-id). A rule carries: `id`, `name`, `isActive`, `targetSlot` (the slot assignment lives in the rule — there is no separate label matrix), `matchField` (registry attribute path matched against the rule's value list), `matchMode` (`values` | `all` — `all` matches every product, no list needed), `valueTemplate` (with `{field}` tokens), optional `fallbackTemplate` (only the first rule of a slot may declare it; used when a rule matched but every template resolved empty). PluginData (scopes client + feed_source) supplies the operational payloads: per-rule value lists (`slotIds: {ruleId → text list}`). Merge semantics: rules union by `id` — ancestor (global) entries keep their positions, more-specific tiers override entries with the same id, unseen entries are appended. Slot evaluation: rules run in merged-list order; per slot, the first rule that matches AND renders a non-empty template value claims the slot for that product (a matched rule whose tokens resolve empty is skipped). Validation: `targetSlot` ∈ `custom_label_0..4` only, match/template fields must be registry paths; label values ≤100 characters are enforced as a QC length-limit warning, not by the plugin. **Scope decision:** slot rules are shared at global/client scope; value lists are per-market (client + feed_source tier, three-tier merge §5.3) — per-market labeling ships with the MVP; ADR-0005 records the pin-on-save trade-off (a feed pins inherited bulk values on its first save). A plugin-local `POST /plugins/custom_labels/preview` route evaluates the UI's draft rules+values against staged products for live match statistics. **Shipped scope:** value-list matching on any registry attribute (`matchField`) plus match-all mode; further condition types (`price_range`, `field_match`, derived buckets) per plugin spec.
