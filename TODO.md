...
**Status:** `[x]` Blocked on the core plugin implementation. When that work begins, create a new plan for these UIs following the same TDD + subagent-driven pattern used in M10-d. The plugin auto-form pattern in `PluginPage` is the starting point; the core-plugin UIs will likely need richer inputs (label multi-select, category tree, rule editor) and a custom JSX renderer on top of the JSON Schema form.

**Done (2026-09-09, M12 category cycle):** the last core plugin UI shipped — CategoryUI (dashboard/rules/manual tabs; AI + Uncategorized as disabled placeholders per spec §9.7). Labelizer/Rules/Filter UIs already existed. Ships with the category plugin (plugins/core/category).
...
### 9.2 [x] usePreview disabled-path test: fake timers instead of the 1500ms blind sleep [P2]

**Done (2026-09-10, `c9f2d73`, section 9 polish cycle):** `usePreview.test.tsx`: `vi.useFakeTimers({ shouldAdvanceTime: true })` added in `beforeEach` with `vi.useRealTimers()` in `afterEach`; disabled-path test uses `vi.advanceTimersByTime(600)` (debounce + margin); slow-response test driven by fake timer advances; stale-response discard logic unchanged.

### 9.3 [x] `labels_equivalence.py`: copy instead of alias fixture rows [P2]

**Done (2026-09-10, `fcc1f3e`, section 9 polish cycle):** `MERGED_SLOT_RULES` rows wrapped in `copy.deepcopy()`; cross-suite mutation structurally impossible.
...
### 9.5 [x] `manifest.py` `config_merge` key-check error message overstates [P2]

**Done (2026-09-10, `bafb058`, section 9 polish cycle):** Error message changed from `"config_merge keys must be non-empty strings"` to `"config_merge keys must be non-empty"`; test assertion updated accordingly.

### 9.6 [x] App-side plugin.py re-exec under `create_app(plugins_dir=...)` [P2]

**Done (2026-09-10, `702f2b1`, section 9 polish cycle):** New test asserts `sys.modules["custom_labels_plugin"]` identity is preserved across `create_app(plugins_dir=...)` lifespan startup; `docs/plugins.md` documents `sys.modules` caching.
...
### 10.1 [x] Fix mypy baseline errors (42) [P2]
...
**References:** `backend/docs/mypy-baseline.md` (historical record of the 42-error baseline and its clusters).
